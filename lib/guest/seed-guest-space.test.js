...
describe('buildGuestSeed', () => {
    ...
    it('gives siblings unique positions', () => {
        const positionKeys = seed.tasks.map((task) => `${task.parent_id ?? `${task.list_id}:${task.sublist_id}`}|${task.position}`);
    ...
    it('schedules recurring tasks days ahead, so the daily cron never spawns copies inside a 30-minute guest', () => {
        ...
        for (const task of recurringTasks) {
            ...
            expect(Date.parse(task.next_occurrence) - NOW.getTime()).toBeGreaterThanOrEqual(24 * 60 * 60 * 1000);
        }
