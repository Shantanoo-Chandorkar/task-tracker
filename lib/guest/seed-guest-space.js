...
const SAMPLE_TASKS = [
    {
        ...
        isPrioritised: true,
        dueInDays: 2,
        description: '<p>Drag a task by its handle to reorder it. Starred tasks float to the top.</p>',
        children: [
    ...
    { listKey: 'launch', sublistKey: 'design', title: 'Write hero copy', status: 'todo', dueInDays: 5 },
    {
        listKey: 'launch',
    ...
    { listKey: 'launch', sublistKey: 'engineering', title: 'Fix login redirect bug', status: 'todo' },
    {
        listKey: 'launch',
...
export function buildGuestSeed({ spaceId, statusIdByCode, now = new Date() }) {
    ...
    const lists = SAMPLE_LISTS.map((sampleList, listPosition) => {
        ...
        return { id, space_id: spaceId, name: sampleList.name, color: sampleList.color, position: listPosition };
    });
    ...
    function addTask(sampleTask, { listId, sublistId, parentId, depth }) {
        ...
        tasks.push({
            ...
            depth,
            position,
            due_date: sampleTask.dueInDays === undefined ? null : toDueDate(now, sampleTask.dueInDays),
...
export async function seedGuestSpace(adminClient, ownerId) {
    ...
    const { error: spaceError } = await adminClient
        .from('spaces')
        .insert({ id: spaceId, name: GUEST_SPACE_NAME, color: GUEST_SPACE_COLOR, position: 0, owner_id: ownerId });
    ...
    const statusIdByCode = Object.fromEntries(
        (statusRows ?? []).filter((statusRow) => statusRow.code).map((statusRow) => [statusRow.code, statusRow.id]),
    );
