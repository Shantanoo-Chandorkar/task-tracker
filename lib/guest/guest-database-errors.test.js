...
describe('toGuestLimitResult', () => {
    it.each(Object.keys(GUEST_LIMITS))('maps a %s limit error to the limit code and its number', (tableName) => {
    ...
    it('uses the singular noun when the limit is one', () => {
        expect(toGuestLimitResult({ message: 'GUEST_LIMIT_REACHED:spaces' }).error).toContain('1 space.');
    });
    ...
    it('returns null for any other error, so the caller keeps its generic message', () => {
        expect(toGuestLimitResult({ message: 'duplicate key value violates unique constraint' })).toBeNull();
