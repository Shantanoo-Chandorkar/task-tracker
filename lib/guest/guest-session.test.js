...
describe('getGuestSecondsLeft', () => {
    it('counts down from the full session length', () => {
        ...
        expect(getGuestSecondsLeft(guestUser, minutesAfterCreation(1))).toBe((GUEST_SESSION_MINUTES - 1) * 60);
    });

    it('rounds down to whole seconds', () => {
        expect(getGuestSecondsLeft(guestUser, createdAtMs + 500)).toBe(GUEST_SESSION_MINUTES * 60 - 1);
    });
    ...
    it('is 0 at and after expiry, never negative', () => {
        ...
        expect(getGuestSecondsLeft(guestUser, minutesAfterCreation(GUEST_SESSION_MINUTES + 30))).toBe(0);
    });
