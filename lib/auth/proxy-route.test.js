...
describe('decideProxyRoute for a logged-out visitor', () => {
    ...
    it.each([
        ...
        '/auth/confirm',
        '/invites/accept',
    ])('lets the public page %s through', (publicPath) => {
...
describe('decideProxyRoute for a registered user (behaviour must match the old proxy)', () => {
    ...
    it('does not bounce the invite-accept page -- a signed-in user is who needs to redeem it', () => {
        expect(decideProxyRoute({ user: realUser, pathname: '/invites/accept' })).toEqual(NEXT);
    });
...
describe('decideProxyRoute for a live guest', () => {
    ...
    it('is not bounced away from /login and /signup, so a guest can move on to a real account', () => {
        expect(decideProxyRoute({ user: guestUser, pathname: '/login', nowMs: liveNow })).toEqual(
            NEXT,
        );
        expect(decideProxyRoute({ user: guestUser, pathname: '/signup', nowMs: liveNow })).toEqual(
            NEXT,
        );
    });
});
...
describe('decideProxyRoute for an expired guest', () => {
    ...
    it('lets a public page through (no redirect loop) but still clears the session', () => {
        expect(
            decideProxyRoute({ user: guestUser, pathname: '/login', nowMs: expiredNow }),
        ).toEqual({
            action: 'next',
            pathname: null,
            search: '',
    ...
    it('treats a guest with an unreadable creation time as expired', () => {
        ...
        expect(
            decideProxyRoute({ user: brokenGuest, pathname: '/', nowMs: createdAtMs })
                .clearsSession,
        ).toBe(true);
    });
});
