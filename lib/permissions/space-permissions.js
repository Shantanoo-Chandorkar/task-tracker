import { PERMISSION_READ_ONLY, PERMISSION_RESTRICTED_NOT_OWN, PERMISSION_NOT_A_MEMBER } from '@/lib/error-codes';
...
export async function resolveSpacePermission(supabase, spaceId, userId) {
    const { data: space } = await supabase.from('spaces').select('owner_id').eq('id', spaceId).maybeSingle();
...
export async function getSpaceIdForList(supabase, listId) {
    const { data: list } = await supabase.from('lists').select('space_id').eq('id', listId).maybeSingle();
...
export function blockCreateForPermission(permissionLevel) {
    if (permissionLevel === 'owner' || permissionLevel === 'full' || permissionLevel === 'restricted') return null;
    if (permissionLevel === 'read_only') {
        return { error: 'Read-only collaborators cannot create new items', code: PERMISSION_READ_ONLY };
    }
...
export async function attachMyPermissionLevel(supabase, spaces, userId) {
    ...
    const permissionLevelBySpaceId = new Map(
        (collaboratorRows || []).map((collaboratorRow) => [collaboratorRow.space_id, collaboratorRow.permission_level]),
    );

    return spaces.map((space) => ({
        ...space,
        my_permission_level: space.owner_id === userId ? 'owner' : (permissionLevelBySpaceId.get(space.id) ?? null),
    }));
}
