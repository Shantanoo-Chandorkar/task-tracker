...
/**
 * Fetches a task's list_id/space_id plus a lightweight id/parent_id view of every task in
 * that list — enough to walk the tree with `findDescendantIds` without pulling full rows.
 *
 * @param {object} supabase - Supabase server client
 * @param {string} taskId - Task whose list-mates are being fetched
 * @returns {Promise<{ task: object|null, listTasks: object[] }>} The task row (with space_id) and its list's tree shape
 */
export async function getTaskListTree(supabase, taskId) {
    ...
    return { task: { list_id: task.list_id, space_id: task.lists?.space_id ?? null }, listTasks };
}
