...
/**
 * Looks up the id of the built-in "done" status.
 *
 * @param {object} supabase - Supabase server client
 * @returns {Promise<string|null>} The done status id, or null if not configured
 */
export async function getDoneStatusId(supabase) {
    const { data } = await supabase.from('statuses').select('id').eq('code', 'done').single();
    return data?.id ?? null;
}

/**
 * Looks up the id of the default status — the target for an "uncomplete" cascade.
 *
 * @param {object} supabase - Supabase server client
 * @returns {Promise<string|null>} The default status id, or null if not configured
 */
export async function getDefaultStatusId(supabase) {
    const { data: defaultStatus } = await supabase.from('statuses').select('id').eq('is_default', true).single();
    return defaultStatus?.id ?? null;
}

/**
 * Fetches a task's list_id plus a lightweight id/parent_id view of every task in that
 * list — enough to walk the tree with `findDescendantIds` without pulling full rows.
 *
 * @param {object} supabase - Supabase server client
 * @param {string} taskId - Task whose list-mates are being fetched
 * @returns {Promise<{ task: object|null, listTasks: object[] }>} The task row and its list's tree shape
 */
