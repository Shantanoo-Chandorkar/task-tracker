...
import { sanitizeString, checkMaxLength, sanitizeRichText, normalizeLinkUrl } from './validation';
...
function findUnsafeMarkup(sanitizedHtml) {
    ...
    for (const element of parsedContainer.querySelectorAll('*')) {
        ...
        for (const attribute of element.attributes) {
            if (!ALLOWED_ATTRIBUTE_NAMES.has(attribute.name))
                unsafeFindings.push(`attribute ${attribute.name}`);
        }

        // Browsers ignore tabs, newlines and spaces inside a scheme, so compare with them removed
        const normalizedHref = (element.getAttribute('href') ?? '')
            .replace(/[\u0000- ]/g, '')
            .toLowerCase();
        if (EXECUTABLE_URL_SCHEME.test(normalizedHref))
            unsafeFindings.push(`href ${normalizedHref}`);
    }
    return unsafeFindings;
}

/**
 * Returns the text a browser would display for the given HTML, with entities decoded.
 *
 * @param {string} html - HTML as stored or sanitized.
 * @returns {string} The visible text.
 */
function getVisibleText(html) {
    const parsedContainer = document.createElement('div');
    parsedContainer.innerHTML = html;
    return parsedContainer.textContent;
}
...
describe('sanitizeRichText', () => {
    ...
    it('strips disallowed tags and attributes', () => {
        expect(sanitizeRichText('<script>alert(1)</script><p onclick="x()">hi</p>')).toBe(
            '<p>hi</p>',
        );
    });

    it('forces rel="noopener noreferrer" on target="_blank" links even if omitted', () => {
        const sanitizedLinkHtml = sanitizeRichText(
    ...
    it('adds rel="noopener noreferrer" to every link, including ones without a target', () => {
        expect(sanitizeRichText('<a href="https://example.com">x</a>')).toContain(
            'rel="noopener noreferrer"',
        );
    });

    it('does not treat literal text that looks like a link tag as a link', () => {
        const sanitizedTextHtml = sanitizeRichText('<p>1 &lt;a b</p>');
        expect(sanitizedTextHtml).not.toContain('rel=');
    });

    it.each([
        ['a < b', 'a &lt; b'],
        ['if a < b and b > c then ok', 'if a &lt; b and b &gt; c then ok'],
        ['x <= y >= z', 'x &lt;= y &gt;= z'],
        ['3 < 5 && 5 > 3', '3 &lt; 5 && 5 &gt; 3'],
    ])(
        'keeps stray angle brackets in "%s" as escaped text instead of deleting them',
        (plainText, expectedHtml) => {
            expect(sanitizeRichText(plainText)).toBe(expectedHtml);
        },
    );

    it.each([
        'a < b',
        'if a < b and b > c then ok',
        '<p>a &lt; b</p>',
        '<p>Tom &amp; Jerry &lt;3</p>',
    ])(
        'renders %s as the same visible text a browser would show, and stays stable when sanitized again',
        (storedText) => {
            const sanitizedOnce = sanitizeRichText(storedText);
            expect(sanitizeRichText(sanitizedOnce)).toBe(sanitizedOnce);
            expect(getVisibleText(sanitizedOnce)).toBe(getVisibleText(storedText));
        },
    );

    it('shows the typed symbols, not entity names, for editor output', () => {
        expect(getVisibleText(sanitizeRichText('<p>a &lt; b &amp; c &gt; d</p>'))).toBe(
            'a < b & c > d',
        );
    });
...
describe('sanitizeRichText XSS payloads', () => {
    const xssPayloads = {
        ...
        'quote-breaking attribute':
            '<a href="https://example.com" title="x" onfocus="alert(1)" autofocus="">x</a>',
        ...
        'script tag with a space after the bracket': '< script>alert(1)</ script>',
    };
    ...
    it.each(Object.entries(xssPayloads))(
        'is idempotent for %s',
        (payloadDescription, xssPayload) => {
...
describe('normalizeLinkUrl', () => {
    it('adds https:// when no scheme is given', () => {
        expect(normalizeLinkUrl('example.com/a')).toEqual({ url: 'https://example.com/a' });
    });

    it('treats host:port as a host, not a scheme', () => {
        expect(normalizeLinkUrl('localhost:3000')).toEqual({ url: 'https://localhost:3000' });
    });

    it('keeps http, https, mailto and tel as typed', () => {
        expect(normalizeLinkUrl(' http://a.com ')).toEqual({ url: 'http://a.com' });
        expect(normalizeLinkUrl('mailto:me@a.com')).toEqual({ url: 'mailto:me@a.com' });
        expect(normalizeLinkUrl('tel:+123456')).toEqual({ url: 'tel:+123456' });
    });
    ...
    it.each([
        ...
        'vbscript:x',
        'ftp://a.com',
    ])('rejects the %s scheme', (unsafeUrl) => {
        expect(normalizeLinkUrl(unsafeUrl).code).toBe('LINK_URL_UNSUPPORTED_PROTOCOL');
    });

    it('rejects empty, spaced, over-long and non-string input', () => {
        expect(normalizeLinkUrl('   ').code).toBe('LINK_URL_REQUIRED');
        expect(normalizeLinkUrl(null).code).toBe('LINK_URL_REQUIRED');
        expect(normalizeLinkUrl('a b.com').code).toBe('LINK_URL_INVALID');
        expect(normalizeLinkUrl(`a.com/${'x'.repeat(2050)}`).code).toBe('LINK_URL_TOO_LONG');
    });
});
