...
describe('sanitizeRichText', () => {
    ...
    it('adds rel="noopener noreferrer" to every link, including ones without a target', () => {
        expect(sanitizeRichText('<a href="https://example.com">x</a>')).toContain('rel="noopener noreferrer"');
    });
    ...
    it.each([
        ...
        ['x <= y >= z', 'x &lt;= y &gt;= z'],
        ['3 < 5 && 5 > 3', '3 &lt; 5 && 5 &gt; 3'],
    ])('keeps stray angle brackets in "%s" as escaped text instead of deleting them', (plainText, expectedHtml) => {
        expect(sanitizeRichText(plainText)).toBe(expectedHtml);
    });

    it.each(['a < b', 'if a < b and b > c then ok', '<p>a &lt; b</p>', '<p>Tom &amp; Jerry &lt;3</p>'])(
    ...
    it('shows the typed symbols, not entity names, for editor output', () => {
        expect(getVisibleText(sanitizeRichText('<p>a &lt; b &amp; c &gt; d</p>'))).toBe('a < b & c > d');
    });
...
describe('normalizeLinkUrl', () => {
    ...
    it.each(['javascript:alert(1)', 'JaVaScRiPt:alert(1)', 'data:text/html,<b>x</b>', 'vbscript:x', 'ftp://a.com'])(
