...
function sortPrioritisedTasksFirst(tasks) {
    return [...tasks.filter((task) => task.is_prioritised), ...tasks.filter((task) => !task.is_prioritised)];
}
...
export function countSublistTasks(sublistId, flatList) {
    ...
    return rootTasks.reduce((total, rootTask) => total + 1 + findDescendantIds(rootTask.id, flatList).size, 0);
}
