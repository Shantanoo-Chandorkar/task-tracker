...
function buildSummaryWithSampleLists({ recentTasks = [], priorityCandidates = [] } = {}) {
    return buildHomeSummary({ recentTasks, priorityCandidates, lists: sampleLists, sublists: sampleSublists });
}

describe('buildHomeSummary', () => {
    it('returns empty sections for empty input', () => {
        expect(buildSummaryWithSampleLists()).toEqual({ priorityTasks: [], recentTasks: [], recentLists: [], recentSublists: [] });
    });
    ...
    it('keeps starred tasks that are not done and drops done ones', () => {
        const summary = buildSummaryWithSampleLists({
            priorityCandidates: [
                ...
                makeTask({ id: 'finished', is_prioritised: true, statuses: { code: 'done', name: 'Done', color: '#0f0' } }),
            ],
        });
    ...
    it('caps priority tasks at 8 and recent tasks at 8, keeping newest-first order', () => {
        const manyTasks = Array.from({ length: 12 }, (_, index) => makeTask({ id: `t${index}`, is_prioritised: true }));
        const summary = buildSummaryWithSampleLists({ recentTasks: manyTasks, priorityCandidates: manyTasks });
        expect(summary.priorityTasks.map((task) => task.id)).toEqual(manyTasks.slice(0, 8).map((task) => task.id));
        expect(summary.recentTasks).toHaveLength(8);
    });

    it('adds the list name and colour and the status to each task', () => {
        const [recentTask] = buildSummaryWithSampleLists({ recentTasks: [makeTask({ id: 'x', list_id: 'listB' })] }).recentTasks;
    ...
    it('skips tasks whose list is not visible', () => {
        const summary = buildSummaryWithSampleLists({ recentTasks: [makeTask({ id: 'orphan', list_id: 'gone' })] });
    ...
    it('ignores a nested task whose root was not fetched, without throwing', () => {
        const summary = buildSummaryWithSampleLists({ recentTasks: [makeTask({ id: 'child', parent_id: 'missing-root' })] });
        expect(summary.recentSublists).toEqual([]);
    });

    it('does not loop forever on a parent cycle', () => {
        const summary = buildSummaryWithSampleLists({
            recentTasks: [makeTask({ id: 'a', parent_id: 'b' }), makeTask({ id: 'b', parent_id: 'a' })],
        });
        expect(summary.recentSublists).toEqual([]);
    });
});
...
describe('loadHomeSummary', () => {
    ...
    it('returns a generic error, not database detail, when a query fails', async () => {
        const supabase = makeFakeSupabase({ data: null, error: { code: '42501', message: 'secret table detail' } });
