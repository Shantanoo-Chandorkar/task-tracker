...
describe('escapeUrl', () => {
    it('escapes the ampersands between query params', () => {
        expect(escapeUrl('https://example.com/a?x=1&y=2')).toBe('https://example.com/a?x=1&amp;y=2');
    });
...
describe('sendEmail', () => {
    ...
    it('returns a failure result instead of throwing when the SMTP send is rejected', async () => {
        ...
        await expect(sendEmail(emailFields)).resolves.toEqual({ success: false, error: 'connection refused' });
    });
    ...
    it('reports an unknown error when the rejection carries no message', async () => {
        ...
        await expect(sendEmail(emailFields)).resolves.toEqual({ success: false, error: 'Unknown error' });
    });
