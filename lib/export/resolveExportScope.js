...
export async function resolveExportScope(supabase, { type, id }) {
    ...
    if (type === 'list') {
        ...
        return {
            ...
            rows: enrichTasks(tasks || [], {
                ...
                sublistNameById: new Map((sublists || []).map((sublist) => [sublist.id, sublist.name])),
                statusNameById,
            }),
        };
    }
    ...
    if (type === 'sublist') {
        ...
        const { data: space } = await supabase.from('spaces').select('name').eq('id', list.space_id).single();
