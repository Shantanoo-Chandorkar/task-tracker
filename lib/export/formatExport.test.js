...
function exportTitleAsCsvRow(taskTitle) {
    const { body: csvBody } = formatExport(
        { scopeName: 'Single task', rows: [{ title: taskTitle }] },
        'csv',
    );
...
describe('formatExport', () => {
    ...
    it('exports whether a task is prioritised, in the column matching the header', () => {
        const { body: csvBody } = formatExport(
            { scopeName: 'Priority', rows: [{ title: 'Starred', is_prioritised: true }] },
            'csv',
        );
        const [headerRow, dataRow] = csvBody.split('\n');
        const priorityColumnIndex = headerRow.split(',').indexOf('is_prioritised');
        expect(priorityColumnIndex).toBeGreaterThan(-1);
        expect(dataRow.split(',')[priorityColumnIndex]).toBe('true');
    });
    ...
    it('writes null, undefined and missing fields as empty CSV cells, but keeps a depth of 0', () => {
        const { body: csvBody } = formatExport(
            {
                scopeName: 'Gaps',
                rows: [{ title: 'Only title', depth: 0, description: undefined }],
            },
            'csv',
        );
        expect(csvBody.split('\n')[1]).toBe(',,,,0,Only title,,,,,');
    ...
    it('builds a safe filename from a scope name full of symbols or path separators', () => {
        expect(formatExport({ scopeName: '../../etc/passwd', rows: [] }, 'csv').filename).toBe(
            'etc-passwd-export.csv',
        );
        expect(formatExport({ scopeName: '!!!', rows: [] }, 'csv').filename).toBe(
            'export-export.csv',
        );
    });
    ...
    it.each(['=', '+', '-', '@'])(
        'prefixes a title starting with %s so spreadsheets treat it as text',
        (formulaTrigger) => {
            expect(exportTitleAsCsvRow(`${formulaTrigger}SUM(A1)`)).toContain(
                `'${formulaTrigger}SUM(A1)`,
            );
        },
    );

    it('keeps the formula neutralised when the title also needs quoting', () => {
        expect(exportTitleAsCsvRow('=HYPERLINK("http://evil","x")')).toBe(
