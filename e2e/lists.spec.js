...
test.describe('lists', () => {
    ...
    test('creating a list shows it under its space', async ({ page }) => {
        ...
        await expect(spaceSection(page, spaceName).getByRole('link', { name: listName })).toBeVisible();
    });
    ...
    test('editing a list renames it', async ({ page }) => {
        ...
        await expect(spaceSection(page, spaceName).getByRole('link', { name: newName })).toBeVisible();
        await expect(spaceSection(page, spaceName).getByRole('link', { name: listName })).toHaveCount(0);
    });

    test('deleting a list shows the cascade count and removes it', async ({ page }) => {
        const listName = await createList(page, spaceName);

        await listRow(page, spaceName, listName).getByRole('button', { name: 'Delete list' }).click();
